...
if [ "$REMOVE_VOLUMES" = true ]; then
    ...
    if docker-compose -f deployment/local/backend.yml -p "$PROJECT_NAME" down -v; then
...
if docker-compose -f deployment/local/backend.yml -p "$PROJECT_NAME" up -d "${SERVICES[@]}"; then
