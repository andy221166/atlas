...
import type { RegisterRequest } from "./user.interface.ts";
...
export const register = async (request: RegisterRequest): Promise<ApiResponse<void>> => {
  const response = await apiClient.post('/api/front/users/register', request)
