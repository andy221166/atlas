<template>
  <div class="shopping-cart p-4 border bg-light shadow-sm rounded">
    ...
    <ul v-if="cart.length" class="list-group mb-3">
      <li v-for="item in cart" :key="item.product.id" class="list-group-item border-0 border-bottom py-3">
        <div class="d-flex align-items-center gap-3">
          <!-- Remove button -->
          <button @click="removeFromCart(item.product.id)" class="btn btn-link text-danger p-0 fs-5"
          ...
          <div class="d-flex align-items-center gap-2">
            <button @click="decreaseQuantity(item)" class="btn btn-sm btn-outline-secondary" :disabled="isProcessing">
              -
            </button>
            <input type="number" v-model="item.quantity" class="form-control form-control-sm text-center"
              style="width: 60px" min="1" @change="updateQuantity(item.product.id, parseInt(item.quantity) || 1)"
              :disabled="isProcessing" />
            <button @click="increaseQuantity(item)" class="btn btn-sm btn-outline-secondary" :disabled="isProcessing">
              +
            </button>
          </div>
    ...
    <button @click="placeOrder" class="btn btn-success w-100 mt-4" :disabled="!cart.length || isProcessing">
