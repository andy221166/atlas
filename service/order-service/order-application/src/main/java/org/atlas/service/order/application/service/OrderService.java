...
public class OrderService {
    ...
    public OrderDto aggregate(Order order, boolean hasUpdateProductPrice) {
        List<OrderDto> orderDtoList = aggregate(Collections.singletonList(order), hasUpdateProductPrice);
    ...
    private Map<Integer, UserDto> fetchUsers(List<Order> orders) {
        List<Integer> userIds = orders.stream()
            .map(Order::getUserId)
            .distinct()
            .toList();
        List<UserDto> users = userServiceClient.listUser(userIds);
