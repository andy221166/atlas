...
import org.atlas.platform.event.core.contract.order.choreography.QuantityReservedEvent;
import org.atlas.platform.event.core.contract.order.choreography.ReserveQuantityFailedEvent;
    ...
    public void handle(OrderCreatedEvent orderCreatedEvent) {
        ...
        try {
            order.getOrderItems().forEach(orderItem -> {
                ...
                int updated = productRepository.decreaseQuantity(orderItem.getProduct().getId(), orderItem.getQuantity());
                if (updated == 1) {
                    log.info("Reserved quantity: productId={}, amount={}, eventId={}",
                        orderItem.getProduct().getId(), orderItem.getQuantity(), orderCreatedEvent.getEventId());
                } else {
                    log.error("Failed to reserve quantity: productId={}, amount={}, eventId={}",
                        orderItem.getProduct().getId(), orderItem.getQuantity(), orderCreatedEvent.getEventId());
                    throw new RuntimeException(String.format("Product %d has insufficient quantity", orderItem.getProduct().getId()));
                }
            });
