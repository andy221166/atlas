...
public class JdbcProductRepository {
  ...
  public long count(SearchProductQuery command) {
    MapSqlParameterSource params = new MapSqlParameterSource();
    String sql = "select count(p.id) from aggProduct p " + buildWhereClause(command, params);
  ...
  public int insertBatch(List<Product> products) {
    ...
    SqlParameterSource[] batch = products.stream()
        .map(aggProduct -> {
          MapSqlParameterSource params = new MapSqlParameterSource();
          params.addValue("name", aggProduct.getName());
          params.addValue("description", aggProduct.getDescription());
          params.addValue("price", aggProduct.getPrice());
          params.addValue("quantity", aggProduct.getQuantity());
          params.addValue("category_id", aggProduct.getCategory().getId());
          return params;
        })
