...
public class QuerySpecification<T> implements Specification<T> {
    ...
    private Predicate createPredicateForFilter(QueryFilter filter, Root<T> root, CriteriaBuilder criteriaBuilder) {
        List<Predicate> predicates = new ArrayList<>();
    ...
    private Predicate createSimplePredicate(QueryFilter.Condition condition, Root<T> root, CriteriaBuilder criteriaBuilder) {
        ...
        return switch (condition.getQueryOperator()) {
            ...
            case GREATER_THAN_EQUAL -> criteriaBuilder.greaterThanOrEqualTo(path, (Comparable) condition.getValue());
            case LESS_THAN_EQUAL -> criteriaBuilder.lessThanOrEqualTo(path, (Comparable) condition.getValue());
            ...
            default -> throw new IllegalArgumentException("Unsupported operation: " + condition.getQueryOperator());
        };
    }
