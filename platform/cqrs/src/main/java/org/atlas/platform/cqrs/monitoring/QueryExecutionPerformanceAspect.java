    ...
    public Object aroundHandle(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            if (timeElapsed <= acceptedMaxExecutionTime) {
                log.info("Finished executing query {}#{}. Elapsed time: {} ms.", className, methodName, timeElapsed);
            } else {
                log.warn("Finished executing query {}#{}. Elapsed time: {} ms ===> Exceeded the maximum time, please check its performance!!!", className, methodName, timeElapsed);
            }
        }
    }
}
