...
public class CommandGateway {
    ...
    private <C extends Command<R>, R> CommandHandler<C, R> obtainExecutor(Command<R> command) {
        if (CollectionUtils.isEmpty(executorMap)) {
            throw new QueryExecutorNotFoundException("Not found executor for command " + command.getClass());
        }
        String executorBeanName = StringUtils.uncapitalize(command.getClass().getSimpleName()) + "Handler";
